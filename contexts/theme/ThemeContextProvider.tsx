...
export const ThemeContextProvider = ({
  ...
  useEffect(() => {
    const getTheme = async () => {
      const userDisplayedTheme = await secureStoreService.getValue(
        SecureStoreKey.USER_DISPLAYED_THEME
      );

      if (userDisplayedTheme) {
        setDisplayedTheme(
          userDisplayedTheme == theme.DARK ? DARK_THEME : LIGHT_THEME
        );
